...
function envoyerDonnee(identif,priorite,codePostal,age,dateTest)
{
    ...
    db = openDatabase('patient', '1.0', "patient table covid 19", 2*1024*1024);
    db.transaction(function (tx) {
    tx.executeSql("CREATE TABLE IF NOT EXISTS patientcovid ( iden INTEGER PRIMARY KEY ,prio INTEGER ,codePostal INTEGER,age INTEGER,dateTest TEXT)");
	  });
	db.transaction(function (tx) {
    tx.executeSql('INSERT INTO patientcovid ( iden,prio,codePostal,age,dateTest) VALUES (?, ?,?,?,?)', [identif,priorite,codePostal,age,dateTest]);
